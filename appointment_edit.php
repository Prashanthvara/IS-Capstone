...
$stmt = $db->prepare('SELECT * FROM appointment, patient WHERE patient.pat_gwid=appointment.pat_gwid AND appointment_id = :id');
$stmt->bindParam(':id', $_GET['id']);
$stmt->execute();
$event = $stmt->fetch();

$stmt2 = $db->prepare('SELECT * FROM  questionnaire WHERE questionnaire.app_id= :id');
...
<div style="padding:10px">
    ...
    <div class="space">
        ...
        <div><input type="text" size="25" id="start" name="start" disabled
                    value="<?php print ($event['pat_firstname']) . ($event['pat_lastname']) ?>"/></div>
    </div>
    <div class="space">
        <div>Patient email:</div>
        <div><input type="text" size="25" id="start" name="start" disabled
                    value="<?php print ($event['pat_email']) ?>"/></div>
    </div>

    <div class="space">
        <div>Appointment Start Time:</div>
    ...
    <div class="space">
        <div>Appointment End Time:</div>
        <div><input type="text" id="end" name="end" disabled
                    value="<?php print (new DateTime($event['appointment_end']))->format('d/M/y g:i A') ?>"/></div>
    </div>
    <div class="space">
        <div>Questionnaire Completed:</div>
        <?php

        if ($event2['questionnaire_id'] > 0) {
            echo "<div><input type=\"text\"  disabled value=\"Yes\" /></div>";
        } else {
            echo "<div><input type=\"text\" disabled value=\"No\" /></div>";
        }
