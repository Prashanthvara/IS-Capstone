...
<td><p><a href=\"  $row->webex_link\" target=\"_blank\" class=\"btn btn-primary btn-sm\">Join</a></p></td></tr>";
                        }
                    }
                    ?>
                    </tbody>
                ...
                <table style="color: black">
                    ...
                    <tbody>
                    <?php
                    $sql = "SELECT doctor.doctor_name, specialty.sp_type, appointment.appointment_patient_name, appointment.appointment_start, appointment.appointment_status FROM appointment, specialty, doctor WHERE specialty.sp_id = doctor.sp_id AND doctor.doctor_id = appointment.doctor_id AND appointment.appointment_status IN('confirmed') AND appointment.appointment_start <= CURRENT_TIME";
                    $res = mysqli_query($db, $sql);
                    if (mysqli_num_rows($res) > 0) {
                        while ($row = mysqli_fetch_object($res)) {
                            echo "<tr><td>" . $row->doctor_name . "</td><td>" . $row->sp_type . "</td><td>" . $row->appointment_patient_name . "</td><td>" . $row->appointment_start . "</td><td>" . $row->appointment_status . "</td><td><p><a href=\"\" class=\"btn btn-primary btn-sm\" style=\"width: 7em;\">Report</a></p></td></tr>";
                        }
                    }
                    ?>
                    </tbody>
